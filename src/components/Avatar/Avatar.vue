<template>
  <div 
    role="img" 
    :class="[contentClass({ size }), 'relative']"
  >
    <span
      :title="name" 
    ...
    >
      ...
      <Anonymous v-else :class="twMerge(contentClass({ size }))" />
    </span>
    <span v-if="name" id="avatar-label" hidden>{{name}}</span>
    <span v-if="presence" :class="twMerge(presenceClass({ appearance, size }))">
      <Busy v-if="presence === 'busy'"/>
      <Online v-else-if="presence === 'online'"/>
      <Offline v-else-if="presence === 'offline'"/>
      <Focus v-else />
    </span>
    <span :class="twMerge(statusClass({ appearance, size }))">
      <Busy v-if="presence === 'busy'"/>
      <Online v-else-if="presence === 'online'"/>
      <Offline v-else-if="presence === 'offline'"/>
      <Focus v-else />
...
import Busy from './presence-icons/Busy.vue';
import Focus from './presence-icons/Focus.vue';
import Online from './presence-icons/Online.vue';
import Offline from './presence-icons/Offline.vue';
...
const statusClass = cva("absolute top-0 bg-[#282E33] rounded-full border-2 border-[#282E33]", {
  variants: {
    appearance: {
      circle: '',
      square: '',
  ...
  compoundVariants: [
    { appearance: "square", size: [ "xs", "sm" ],  class: "-right-0.5 -top-0.5" },
    { appearance: "square", size: [ "md", "lg", "xl", "2xl" ],  class: "-right-1 -top-1" },
  ]
});

const presenceClass = cva("absolute bottom-0 bg-[#282E33] rounded-full border-2 border-[#282E33]", {
  variants: {
    appearance: {
      circle: '',
      square: '',
  ...
  compoundVariants: [
    { appearance: "square", size: [ "xs", "sm" ],  class: "-right-0.5 -bottom-0.5" },
    { appearance: "square", size: [ "md", "lg", "xl", "2xl" ],  class: "-right-1 -bottom-1" },
  ]
});

const avatarContainerClass = cva("inline-flex static items-center justify-center select-none shrink-0 bg-slate-200 overflow-hidden text-slate-950", {
  variants: {
    appearance: {
      circle: [ "rounded-full" ],
      square: [ "rounded" ],
